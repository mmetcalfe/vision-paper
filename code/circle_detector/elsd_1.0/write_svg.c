  ...
  if (angle_diff(ang_start,ang_end)<M_2__PI*SQRT2/C && angle_diff_signed(ang_start,ang_end)>0)
    {
      ...
      fprintf(fe,"- { translate: ['%f', '%f'], rotate: '0', size: ['%f', '%f']}\n",param[0],param[1],param[2],param[3],1);
      return; // EDIT: Output simpler ellipse descriptions.
    }
  else return; // EDIT: Ignore circles that are not full circles.
    {
      double x1,y1,x2,y2;
  ...
  if (angle_diff(ang_start,ang_end)<M_2__PI*SQRT2/C && angle_diff_signed(ang_start,ang_end)>0)
    {
      ...
      fprintf(fe,"- { translate: ['%f', '%f'], rotate: '%f', size: ['%f', '%f']}\n",param[0],param[1],(param[4])*180/M_PI,param[2],param[3],1);
      return; // EDIT: Output simpler ellipse descriptions.
    }
  else return; // EDIT: Ignore ellipses that are not full circles.
    {
      double x1,y1,x2,y2;
      if (ang_start<0) ang_start+=M_2__PI;
      if (ang_end<0) ang_end+=M_2__PI;
